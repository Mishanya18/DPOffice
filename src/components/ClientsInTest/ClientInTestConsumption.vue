...
export default {
  ...
  methods: {
    ...
    cookGridData() {
      // this.priceListNumber = this.$store.getters.clientByCode(
      //   this.clientCode
      // ).PRICE_LIST;
      this.summa = 0;
      this.gridDataReportClient = [];
      this.clientService.forEach((service) => {
        ...
        this.$store
          ...
          .then((res) => {
            partnerService = res;
            //Определение цены исходя из Price List'а
            let price = partnerService.PRICE;
            // if (this.priceListNumber == 1) {
            //   price = partnerService.PRICE;
            // } else if (this.priceListNumber == 2) {
            //   price = partnerService.PRICE_PL2;
            // }
