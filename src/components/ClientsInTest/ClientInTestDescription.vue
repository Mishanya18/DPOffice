<template>
  <el-container direction="vertical">
    ...
    <client-in-test-add-edit
      v-if="showEditDialog"
      :dialogFormVisible="showEditDialog"
      :forma="form"
      @dialogClose="showEditDialog = false"
...
<script>
import ClientInTestAddEdit from "./ClientInTestAddEdit.vue";
...
export default {
  ...
  methods: {
    ...
    showEditClientDialog() {
      this.form.title = "Изменить тест";
      this.form.code = this.clientParams.code;
      this.form.short_name = this.clientParams.name;
      this.form.full_name =
        this.clientParams.full_name == "Не указано"
          ? ""
          : this.clientParams.full_name;
      this.form.inn = this.clientParams.inn;
      let options = [];
      if (this.dealCont[0] != "Не указано") {
        this.form.cont_deal = this.clientParams.deal_cont;
      ...
      if (this.techCont[0] != "Не указано") {
        this.form.cont_tech = this.clientParams.tech_cont;
      ...
      this.form.manager = this.clientParams.manager_id;
      this.form.isClientEdit = true;
          ...
          fetch(
            ...
            .then((data) => {
              if (data.error) {
                ...
                this.$router.push({ name: "clientsintest" });
                this.$message({
  ...
  async mounted() {
    if (Object.keys(this.$store.getters.currentUser).length != 0) {
      let user = this.$store.getters.currentUser;
      if (user.RIGHT == "FullAccess" && user.ISLOGGEDIN == true) {
        this.right = 1;
      }
    } else {
      this.right = 0;
